...
accelerate launch revise/dpo.py \
    --run_name "step-1-dpo-verification" \
    --model_name_or_path "${TEMP_PATH}/step-0-sft" \
    --dataset_path "${HUB_USER_ID}/gsm8k" \
    --dataset_name "llama-3.2-1b-step-1" \
    --output_dir "${TEMP_PATH}/step-1-dpo" \
    ...
    --push_to_hub false \
    --hub_strategy end \
    --hub_model_id "${HUB_USER_ID}/revise-gsm8k-llama-3.2-1b-step-1-dpo" \
